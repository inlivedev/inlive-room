...
const orderStreams = (
	...
	streams.sort((streamA, streamB) => {
	  ...
	  if (streamA.source=== 'screen')
		  return -1;
	  if (streamB.source=== 'screen')
		  return 1;
...
export default function Conference(){
  ...
  const removeStream = (stream: ParticipantVideo) => {
	setStreams((prevStreams) => prevStreams.filter((prevStream) => prevStream.id !== stream.id));
  }
  ...
  useEffect(() => {
    function layoutVideo() {
		...
		if (currentLayout === 'presentation'){
			let style;
			if(layoutContainerRef.current.clientWidth>layoutContainerRef.current.clientHeight){
				// landscape
				console.log('landscape layout',layoutContainerRef.current.clientWidth,layoutContainerRef.current.clientHeight);
			style = {
				display: 'grid',
				gap: '1rem',
				gridTemplateColumns: `5fr 1fr`,
				gridTemplateRows: `repeat(${MAX_VISIBLE_PARTICIPANTS-1}, minmax(0, 1fr))`,
			  };
			} else {
				// portrait
				console.log('portrait layout',layoutContainerRef.current.clientWidth,layoutContainerRef.current.clientHeight);
				style = {
					display: 'grid',
					gap: '1rem',
					gridTemplateRows: `5fr 1fr`,
					gridTemplateColumns: `repeat(${MAX_VISIBLE_PARTICIPANTS-1}, minmax(0, 1fr))`,
				  };
			}
			  setStyle(style);
		} else {
  ...
  return (
	<div className="viewport-height grid grid-cols-[1fr,auto]">
      <div className="relative grid h-full grid-rows-[auto,1fr,72px] overflow-y-hidden">
        ...
        <div className="px-4">
			<div className="relative grid h-full w-full grid-cols-[auto,minmax(auto,max-content)]">
				<div className="grid grid-rows-[auto,1fr]">
					...
					className={currentLayout + '-layout participant-container absolute h-full w-full'}
					style={style}
					>
					{streams.map((stream, id) => {
						...
						if(!hidden && layoutContainerRef.current && (stream.source==='screen' || stream.spotlight || stream.pin)){
							if(layoutContainerRef.current.clientWidth>layoutContainerRef.current.clientHeight){
								// landscape
								style = {
									display: 'grid',
				...
				<div className="ml-4 w-[360px]">
					...
					{sidebar === 'participants' ? <ParticipantListSidebar streams={streams} /> : null}
        ...
        <ConferenceActionsBar streams={streams} sidebar={sidebar} deviceTypes={deviceTypes} />
      </div>
    </div>
  );
};
